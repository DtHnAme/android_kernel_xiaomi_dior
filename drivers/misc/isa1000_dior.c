...
static const unsigned int pwm_period_ns = NSEC_PER_SEC / PWM_FREQUENCY;
...
static int isa1000_set_state(struct isa1000_vib *vib, int on)
{
	...
	if (on) {
		rc = pwm_config(vib->pwm, 
		                (pwm_period_ns  * vib->pwm_duty_percent) / 100, 
		                pwm_period_ns);
		if (rc < 0) {
...
static ssize_t isa1000_amp_show(struct device *dev, 
...
static ssize_t isa1000_amp_store(struct device *dev, 
...
static int isa1000_parse_dt(struct platform_device *pdev, struct isa1000_vib *vib)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, 
	           "timeout-ms", &vib->timeout);
	if (ret < 0)
		dev_err(&pdev->dev, "please check timeout");

	ret = of_property_read_u32(pdev->dev.of_node, "pwm-ch-id", &vib->pwm_ch_id);
	if (ret < 0)
		dev_err(&pdev->dev, "please check pwm output channel");

	/* print values*/
	dev_info(&pdev->dev, "gpio-isa1000-en: %i, gpio-haptic-en: %i, timeout-ms: %i, pwm-ch-id: %i", 
				vib->gpio_isa1000_en, vib->gpio_haptic_en, vib->timeout, vib->pwm_ch_id);
...
static int isa1000_probe(struct platform_device *pdev)
{
	...
	vib->pwm = pwm_request(vib->pwm_ch_id, "isa1000");
	if (IS_ERR(vib->pwm)) {
		dev_err(&pdev->dev,"pwm request failed");
		return PTR_ERR(vib->pwm);
	}

	vib->vdd_regulator = regulator_get(NULL,"8226_l28");
	if (IS_ERR(vib->vdd_regulator)) {
		pr_err("%s:fail to get vdd l28\n",__func__);
		return PTR_ERR(vib->vdd_regulator);
	}
	ret = regulator_enable(vib->vdd_regulator);
	if (ret) {
